...
$query='SELECT * FROM categorie_mission';
$resultSet = $pdo->query($query);
          ...
          <tbody>
           ...
           <tr>
            ...
            <?php if($colonnes['statut'] == "0"){ ?>
              ...
              <option value='Non debute'
                    <?php if($controle['statut'] == "Non debute"){echo 'selected="selected"';}?>>Non debute</option>
              <option value='Documente'
                    <?php if($controle['statut'] == "Documente"){echo 'selected="selected"';}?>>Documente</option>
                    <option value='Revu' <?php if($controle['statut'] == "Revu"){echo 'selected="selected"';}?>>Revu</option>
                    <?php }else{?>
                      <span class='search-statut span-statut search-recherche'><?php echo $controle['statut'] ;?></span>
              <select class='statut-select' data-id='<?php echo $controle['id'] ;?>'> 
              <option value='Revu'
                    <?php if($controle['statut'] == "Revu"){echo 'selected="selected"';}?>>Revu</option>
  ...
  $.ajax({
            ...
            success: function(data) {
              ...
              for (var i = 0; i < data.length; i++) {
                div_list_commentaire.innerHTML += '<div class="commentaire-div"><p>'+data[i].date_commentaire+', '+data[i].nom+ ' '+ data[i].prenom + ' : '+ data[i].commentaire+ '</p><div class="textarea"><textarea>'+data[i].commentaire+'</textarea><button type="button" class="maj">Mettre a jour</button></div><button type="button" class="edit-bouton btn btn-primary" data-id="'+data[i].id+'"><img src="bootstrap-icons-1.4.0/pencil.svg"></button><button type="button" class="delete-bouton btn btn-danger" data-id="'+data[i].id+'"><img src="bootstrap-icons-1.4.0/trash.svg"></button></div>';
              }
